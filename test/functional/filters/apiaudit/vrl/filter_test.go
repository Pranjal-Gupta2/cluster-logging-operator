...
var _ = Describe("Policy to VRL Filter", func() {
	...
	Context("omit response codes", func() {
		It("should omit specified codes", func() {
			p := &loggingv1.APIAudit{OmitResponseCodes: &[]int{1234}}
		...
		It("should not omit by code if explictly empty", func() {
			p := &loggingv1.APIAudit{OmitResponseCodes: &[]int{}} // Explictly Empty
	...
	Context("omit stages", func() {
		It("omits using policy setting", func() {
			p := &loggingv1.APIAudit{OmitStages: []Stage{StageRequestReceived}}
		...
		It("omits using rule setting", func() {
			p := &loggingv1.APIAudit{Rules: []PolicyRule{{Verbs: []string{"update"}, OmitStages: []Stage{StagePanic}}}}
			Expect(Filtered(p, Event{Verb: "update", Stage: StagePanic})).To(BeNil())
			Expect(Filtered(p, Event{Verb: "get", Stage: StagePanic})).NotTo(BeNil())
		})
		It("omits using rule and policy setting", func() {
			...
			Expect(Filtered(p, Event{Verb: "get", Stage: StageRequestReceived})).To(BeNil())
	...
	Context("policy rules", func() {
		It("matches by verb", func() {
			...
			Expect(Filtered(p, Event{Verb: "foo"})).To(HaveLevel(LevelRequestResponse))
		})
		...
		It("matches by namespace", func() {
			...
			Expect(Filtered(p, Event{ObjectRef: &ObjectReference{Namespace: "other"}})).To(HaveLevel(LevelRequestResponse))
		})
		...
		It("matches by resource", func() {
			...
			Expect(Filtered(p, Event{ObjectRef: &ObjectReference{APIGroup: "route.openshift.io", Name: "foobar", Resource: "routes"}})).To(HaveLevel(LevelRequestResponse))
			Expect(Filtered(p, Event{ObjectRef: &ObjectReference{APIGroup: "batch", Resource: "foo/status"}})).To(HaveLevel(LevelMetadata))
			Expect(Filtered(p, Event{ObjectRef: &ObjectReference{APIGroup: "batch", Resource: "foo"}})).To(HaveLevel(LevelRequestResponse))
			...
			Expect(Filtered(p, Event{})).To(HaveLevel(LevelRequestResponse))
			Expect(Filtered(p, Event{ObjectRef: nil})).To(HaveLevel(LevelRequestResponse))
			Expect(Filtered(p, Event{ObjectRef: &ObjectReference{}})).To(HaveLevel(LevelRequestResponse))
		})
		...
		It("matches by non-resource", func() {
			...
			Expect(Filtered(p, Event{RequestURI: ""})).To(HaveLevel(LevelRequestResponse))
			Expect(Filtered(p, Event{})).To(HaveLevel(LevelRequestResponse))
		})
