...
var _ = BeforeSuite(func() {
	if testfw.LogCollectionType != constants.VectorName {
		Skip("only works with vector collector")
	}
	c = client.NewTest()
	name := "apiaudit-test"
...
func Filtered(policy *loggingv1.APIAudit, event Event) *Event {
	fillEvent(&event)
...
func FilteredBytes(policy *loggingv1.APIAudit, b []byte) []byte {
	cmd := vectorCmd(policy)
	cmd.Stdin = bytes.NewReader(b)
	out, err := cmd.Output()
	test.Must(utils.WrapError(err))
	return out
}

func vectorCmd(p *loggingv1.APIAudit) *exec.Cmd {
...
encoding.codec = "json"
`, vrl)
	Expect(testruntime.PodWrite(pod, "", "/tmp/vector.toml", []byte(conf))).To(Succeed())
...
func readPolicy(path string) *loggingv1.APIAudit {
	b, err := os.ReadFile(path)
	test.Must(err)
	policy := &loggingv1.APIAudit{}
